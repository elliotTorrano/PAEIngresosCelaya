"""Pantalla del Reporteador para el reporte general de Requerimientos:
concentra, por FOLIO, lo que trae la lista de origen (importada aquí mismo,
con ADEUDO) y lo que el Agente exporta al terminar "Revisar Formato"
(Despacho + captura del Abogado). El resto de las columnas (domicilio de
notificación, fechas de entrega/recepción, observaciones del área, fecha
extrajudicial, motivo de suspensión) se llenan a mano directamente aquí.

Cada cambio (import, edición manual, asignar fecha de entrega) regenera por
completo el .xlsx maestro (si hay uno configurado) -- no hay edición
incremental de archivos .xlsx en este programa, y para los volúmenes de esta
app regenerarlo completo es instantáneo."""
...
from app.excel_io.reporte_requerimientos_export import HEADERS, export_reporte_xlsx
from app.excel_io.reporte_requerimientos_import import parse_revision_file, parse_source_file
...
(
    COL_LISTA, COL_FOLIO, COL_CTA, COL_CONTRIB, COL_DOM_UBICACION, COL_DOM_NOTIFICACION,
    COL_ADEUDO, COL_DESPACHO, COL_FECHA_IMPRESO, COL_FECHA_ENTREGA, COL_FECHA_RECEPCION,
    COL_FECHA_CITATORIO, COL_QUIEN_CITATORIO, COL_FECHA_DILIGENCIA, COL_CON_QUIEN_NOTIFICO,
    COL_OBS_ABOGADO, COL_OBS_AREA, COL_FECHA_EXTRAJUDICIAL, COL_MOTIVO_SUSPENSION,
) = range(19)

# Columnas que sólo se llenan a mano en esta pantalla -- (columna, nombre del
# campo en reporte_requerimientos_rows, texto de marcador).
MANUAL_CELLS = (
    (COL_DOM_NOTIFICACION, "domicilio_notificacion", ""),
...
class ReporteRequerimientosView(QWidget):
    def __init__(self, user, parent=None):
        super().__init__(parent)
        self.user = user
        self._rows: list[reporte_repo.ReporteRequerimientoRow] = []

        layout = QVBoxLayout(self)
        layout.addWidget(QLabel(
            "Reporte general de Requerimientos: importe primero la(s) lista(s) de origen "
    ...
    def _fill_row(self, r: int, row: reporte_repo.ReporteRequerimientoRow) -> None:
        manual_cols = {col for col, _field, _placeholder in MANUAL_CELLS}
        values = [
            row.lista_numero, row.folio, row.cta_predial, row.contribuyente,
            row.domicilio_ubicacion, row.domicilio_notificacion, row.adeudo, row.despacho,
            row.fecha_impreso, row.fecha_entrega, row.fecha_recepcion, row.fecha_citatorio,
            row.quien_recibe_citatorio, row.fecha_diligencia, row.con_quien_notifico,
            row.observaciones_abogado, row.observaciones_area, row.fecha_extrajudicial,
    ...
    def _update_master_path_label(self) -> None:
        path = settings_repo.get(settings_repo.KEY_REPORTE_REQUERIMIENTOS_EXCEL_PATH)
    ...
    def _on_choose_master_file(self) -> None:
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Elegir archivo maestro del reporte", "Reporte General Requerimientos.xlsx", "Excel (*.xlsx)"
        )
        if not file_path:
            return
        settings_repo.set(settings_repo.KEY_REPORTE_REQUERIMIENTOS_EXCEL_PATH, file_path)
        self._update_master_path_label()
        self._sync_master_file()
    ...
    def _on_export_copy(self) -> None:
        if not self._rows:
            ...
            return
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Exportar copia del reporte", "Reporte General Requerimientos.xlsx", "Excel (*.xlsx)"
        )
        if not file_path:
            return
